...
中文管理后台：配置热点与采集备注、为调用方分配 API key、归档和搜索热点信息、通过企业微信群机器人发送订阅通知。采集、定时调度和 AI 总结由外部调用方完成。
...
- 热点信息：按热点与日期筛选、搜索标题/总结/正文、查看详情及原文。
- 调用方：生成、重置和禁用 key，查看最后调用时间。
- 订阅方：选择热点或订阅全部，key 加密保存，发送测试通知。
