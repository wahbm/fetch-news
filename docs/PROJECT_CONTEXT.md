...
本轮开发已完成并通过本地验证。代码改动已提交到 `3ac88903`；本次交接文档是随后产生的工作区改动。此时 `main` 比 `origin/main`（`6077dcab`）领先 1 个代码提交，`3ac88903` 尚未推送或部署。线上仍是首次发布的 `eae80c2`，因此线上尚未具备本轮的 `heat_score` 和批量接口，生产数据库也不应被本地测试连接。

线上交接事实：公网地址为 `https://8.130.116.192/davyluiy/fetch-news/`，Nginx 终止 HTTPS，Node/systemd 仅监听 `127.0.0.1:3107`，候选端口为 `3108`；项目目录 `/srv/fetch-news`，服务 `fetch-news.service`，数据库 `fetch_news`。用户明确选择不配置数据库备份，生产未录入真实热点、调用方或企业微信 key。
