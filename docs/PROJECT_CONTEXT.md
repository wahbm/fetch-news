...
本轮开发已完成并通过本地与 CI 验证。通知格式提交 `7f15905f` 已推送到 `main` 并于 2026-09-14 发布到 ECS；当前线上已执行 `002_article_heat_score.sql`，具备 `heat_score`、批量接口和微信友好通知卡片；生产数据库仍不得用于本地测试。

线上交接事实：公网地址为 `https://8.130.116.192/davyluiy/fetch-news/`，Nginx 终止 HTTPS，Node/systemd 仅监听 `127.0.0.1:3107`，候选端口为 `3108`；项目目录 `/srv/fetch-news`，服务 `fetch-news.service`，数据库 `fetch_news`。当前运行提交 `7f15905f`，上一正常版本为 `3ac88903`。用户明确选择不配置数据库备份，生产未录入真实热点、调用方或企业微信 key。
